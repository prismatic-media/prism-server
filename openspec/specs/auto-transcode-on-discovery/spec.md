# auto-transcode-on-discovery Specification
...
The system SHALL support a configurable setting `auto_transcode_on_discovery` with valid values `"true"` and `"false"` (default `"false"`). When `"true"`, the system SHALL automatically enqueue a transcode job for each newly discovered media item that has no existing transcode job.
